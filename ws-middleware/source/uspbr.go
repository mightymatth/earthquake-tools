...
func NewUspBr() UspBr {
	return UspBr{
		NewFdsnWs("IRIS", "https://service.iris.edu/fdsnws/event/1/query", UspBrID),
	}
}
...
func (s UspBr) Transform(r io.Reader) ([]EarthquakeData, error) {
	...
	for _, feature := range features {
		data := EarthquakeData{
			...
			DetailsURL: fmt.Sprintf("http://www.moho.iag.usp.br/eq/event/%s", uspBrEventID(feature.PublicID)),
			SourceID:   s.SourceID,
			EventID:    uspBrEventID(feature.PublicID),
		}
...
func uspBrEventID(publicIDPath string) string {
...
type uspBrTime time.Time

func (c *uspBrTime) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	var v string
