...
// EqEventHandler is a handler for incoming earthquake events via HTTP.
...
// EqEventSender is a handler for incoming earthquake events via channel.
func EqEventSender(events <-chan source.EarthquakeData, bot *tgbotapi.BotAPI, s storage.Service) {
	for event := range events {
		go sendEventReport(EarthquakeEvent{
			Mag:        event.Mag,
	...
	log.Fatalf("stopped receiving events")
}
...
func sendEventReport(event EarthquakeEvent, bot *tgbotapi.BotAPI, s storage.Service) {
	eventData := entity.EventData{
		...
		Source: entity.SourceID(event.SourceID),
	}
